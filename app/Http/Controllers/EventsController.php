...
*/
		    ...
		    if(isset($registrant)) unset($registrant);
    ...
    public function registerConfirm(Event $event, Registrant $registrant, $team=null, $donation=null, $user=null) {
	    if(!is_null($team) && $team!=0) { $team = Team::where('id',$team)->first(); } else { $team=null; }
	    if(!is_null($donation) && $donation!=0) { $donation = Donation::where('id',$donation)->first(); } else { $donation=null; }
	    if(!is_null($user) && $user) { $user = User::where('slug',$user)->first(); } else { $user=null; }
	...
	public function registrantSearchResult(Request $request) {
		...
		print view('parts.registrantSearchResult',compact('registrants'));
		die();
	}
}