...
class DonateController extends Controller
{
    ...
    public function donate(Request $request) {
		...
		$check = Donation::where('fname',$request->fname)->where('lname',$request->lname)->where('email',$request->email)->where('amount',str_replace(array('$',','),array('',''),$request->amount))->orderBy('created_at','DESC')->first();
		if(empty($check) || strtotime($check->created_at)<strtotime("2 minutes ago")) {
	        ...
	        $cost = str_replace(array('$',','),array('',''),$request->amount);
	        $plan = '';
	        $customer = '';
	        $subscription = '';
	        if($cost==0) {
		        $cost = str_replace(array('$',','),array('',''),$request->otheramount);
	        }
