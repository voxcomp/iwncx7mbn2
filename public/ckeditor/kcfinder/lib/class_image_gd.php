...
class image_gd extends image
{
    ...
    public function resizeFit($width, $height, $background = false)
    {
        ...
        if ($background === false) {
            ...
            if (($this->resize($w, $h) === false) ||
                ($img->imageFilledRectangle(0, 0, $width, $height, $background) === false) ||
                ($img->imageCopyResampled($this->image, $x, $y, 0, 0, $w, $h) === false)
            ) {
                return false;
            }
    ...
    protected function getImage($image, &$width, &$height)
    {
        ...
        if (is_resource($image) && (get_resource_type($image) == 'gd')) {
            ...
            (false !== ([$width, $height, $t] = @getimagesize($image)))
        ) {
            $image =
    ...
    {
        if (! is_string($file) ||
            ((false === ([$width, $height, $t] = @getimagesize($file))))
        ) {
            return false;
        }
    ...
    protected function imageCopyResampled(
        ...
        [$src, $srcWidth, $srcHeight] = $imageDetails;
